...
def garm_app_image_fixture(pytestconfig: pytest.Config) -> str | None:
    ...
    return pytestconfig.getoption(GARM_IMAGE_PARAM)


@pytest.fixture(scope="module", name="garm_app")
def deploy_garm_app_fixture(
    juju: jubilant.Juju,
    garm_charm_file: str,
    garm_app_image: str,
) -> str:
    """Deploy the GARM application and wait for it to become active.

    - Deploys the GARM charm with the provided ROCK image as the app-image resource.
    - Waits for the application to reach active status. GARM requires no external
      integrations at the scaffold stage: secrets are auto-generated by the leader
      unit and all config options have sensible defaults.
