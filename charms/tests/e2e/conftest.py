...
from tests.integration.helpers import required_env
...
def integrate_garm_ingress_fixture(
    ...
    # Assert GARM's controller-info reports metadata_url on the LB IP
    resp = requests.get(
...
def deploy_e2e_scaleset_fixture(
    ...
    # Decode base64 private key before creating the secret so GARM receives PEM
    private_key_raw = required_env("TEST_GITHUB_APP_PRIVATE_KEY")
    try:
        private_key_decoded = base64.b64decode(private_key_raw).decode()
    except Exception:
        private_key_decoded = private_key_raw
