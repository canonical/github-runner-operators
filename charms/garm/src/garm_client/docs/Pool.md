...
**flavor** | **str** |  | [optional] 
**forge_instance_id** | **str** |  | [optional] 
**generation** | **int** | Generation holds the numeric generation of the pool. This number will be incremented, every time certain settings of the pool, which may influence how runners are created (flavor, specs, image) are changed. When a runner is created, this generation will be copied to the runners as well. That way if some settings diverge, we can target those runners to be recreated. | [optional] 
...
**priority** | **int** | Priority is the priority of the pool. The higher the number, the higher the priority. When fetching matching pools for a set of tags, the result will be sorted in descending order of priority. | [optional] 
**provider_name** | **str** |  | [optional] 
**proxy_id** | **int** | ProxyID is the ID of the proxy definition that will be used by runners spawned in this pool. Runners will use the proxy settings to reach back to GARM, the forge and any other resources they need during setup. | [optional] 
