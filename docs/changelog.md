...
- `garm`: on the first upgrade to the release carrying the change above, every existing scale set is replaced rather than renamed in place, because its un-suffixed name is not a generation of anything. The replacement is created and the old scale set is disabled, drained, and deleted in the same reconcile, so unlike a label change this one has a brief window where no scale set is listening for the labels; queued jobs wait for the replacement instead of being lost. A runner part-way through a job is left to finish and removed on a later pass, so no in-flight job is cut short. No operator action is required.
