...
> asserts nothing beyond the tenant credentials reaching pytest; the deployment and the
> end-to-end assertions land in a follow-up. The workflows are here first because
> `workflow_dispatch` does not work until they are on the default branch.
...
Both workflow files must be on the default branch before dispatch works — the dispatch
endpoint 404s otherwise. Once they are, `--ref` selects which branch's version runs, so a
feature branch needs no further merge. The exception is
`garm_e2e_test_run.yaml`'s `workflow_dispatch.inputs`: changing that contract is the one
edit not reliably testable from a branch, so treat it as settled.
