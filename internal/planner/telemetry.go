...
func NewMetrics(store FlavorStore) *Metrics {
	...
	must(
		meter.RegisterCallback(
			m.collectFlavorPressureMetrics,
...
// collectFlavorPressureMetrics is the callback function that collects and observes
// flavor pressure metrics from the database for all platforms.
func (m *Metrics) collectFlavorPressureMetrics(ctx context.Context, observer metric.Observer) error {
	m.mu.RLock()
	defer m.mu.RUnlock()
